...
//				.assignTimestampsAndWatermarks(waterMark());
        final DataStream<JsonNode> mainStream = env.fromSource(kafkaSource, this.waterMark(), "Kafka")
                                                   .rebalance()
                                                   .assignTimestampsAndWatermarks(this.waterMark());

        final DataStream<String> refStream = env.fromElements("John", "Zbe", "Abe")
                                                .assignTimestampsAndWatermarks(this.waterMark());

        mainStream.join(refStream)
